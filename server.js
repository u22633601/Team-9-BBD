...
io.on("connection", (socket) => {
    ...
    if (game) {
      if (
        ...
        console.log(
          "gameId:",
          gameId,
          "\t|\tViewer joined the game, username: ",
          username,
        );
      }
    } else {
  ...
  socket.on('startGame', (gameId) => {
    ...
    if (game && socket.id === game.host) {
        ...
        game.players.forEach(player => {
            player.socket.emit('initGameState', {
                ball: ball,
                hole: hole,
                maze: maze.map,
                timeLeft: timeLeft,
        ...
        // Emit game state to viewers (without imposter information)
        game.viewers.forEach(viewerUsername => {
            const viewerSocket = io.sockets.sockets.get(game.sockets.find(s => s.id === viewerUsername).id);
            if (viewerSocket) {
                viewerSocket.emit('initGameState', {
                    ball: ball,
                    hole: hole,
                    maze: maze.map,
                    timeLeft: timeLeft
        ...
        if (timeLeft <= 0) {
          ...
          io.to(gameId).emit("gameOver", { win: false, players: game.players.map(player => player.username) }); // Stop the game loop
          ...
          console.log(
            "Game ID: ",
            gameId,
            " | Ball reached the hole, game over (win)",
          );
          io.to(gameId).emit("gameOver", { win: true, players: game.players.map(player => player.username) });
  ...
  socket.on('guessImposter', (guess) => {
    const game = Array.from(games.values()).find(g => g.players.some(p => p.socket === socket));
    if (game) {
        const correct = guess === game.imposter;
        socket.emit('imposterResult', { correct, imposter: game.imposter });
    }
});
